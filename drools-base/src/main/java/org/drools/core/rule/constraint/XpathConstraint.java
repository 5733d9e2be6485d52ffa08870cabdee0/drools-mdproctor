...
public class XpathConstraint extends MutableTypeConstraint {
    ...
    @Override
    public boolean isAllowed(FactHandle handle, ValueResolver reteEvaluator) {
        throw new UnsupportedOperationException();
    }
    ...
    private interface XpathEvaluator {
        Iterable<?> evaluate(ValueResolver valueResolver, BaseTuple tuple, Object object);
    }
    ...
    private static class SingleChunkXpathEvaluator implements XpathEvaluator {
        ...
        public Iterable<?> evaluate(ValueResolver valueResolver, BaseTuple tuple, Object object) {
            return evaluateObject(valueResolver, tuple, chunk, new ArrayList<Object>(), object);
        }

        private List<Object> evaluateObject(ValueResolver valueResolver, BaseTuple leftTuple, XpathChunk chunk, List<Object> list, Object object) {
            Object result = chunk.evaluate(object);
            if (!chunk.lazy && result instanceof ReactiveObject) {
                ((ReactiveObject) result).addTuple(leftTuple);
            }
            if (chunk.iterate && result instanceof Iterable) {
                for (Object value : (Iterable<?>) result) {
                    if (!chunk.lazy && value instanceof ReactiveObject) {
                        ((ReactiveObject) value).addTuple(leftTuple);
                    }
                    if (value != null) {
                        list.add(value);
                    }
